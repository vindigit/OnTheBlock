...
describe('debt payment engine', () => {
  it('pays debt from cash and logs the payment', () => {
    const run = createNewRun({ seed: 'debt-payment' });
  ...
  it('allows max payment bounded by available cash and debt', () => {
    const cashBoundRun = { ...createNewRun({ seed: 'cash-bound' }), cash: 250, debt: 500 };
    ...
    const debtBoundRun = { ...createNewRun({ seed: 'debt-bound' }), cash: 500, debt: 250 };
  ...
  it('rejects invalid, impossible, and ended-run payments', () => {
    const run = createNewRun({ seed: 'reject-debt' });
